...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            char method[4], target[line_len - 14], version[9];

            for(int i = 0 ; i < 3 ;++i)
                method[i] = line[i];
            method[3] = '\0'; 

            for(int i = 0 ; i < (line_len - 15 );++i)      
                target[i] = line[i + 4];
            target[line_len - 15] = '\0';

            for(int i = 0 ; i < 8 ;++i)              
                version[i] = line[line_len - 10 + i]; 
            version[8] = '\0';

            if (line[3] != ' ' && line[line_len - 11] != ' ')
            {
                error(400);
                continue;
            }

            if (strchr(target , ' ') != NULL && strchr(method , ' ') != NULL \
                    && strchr(version, ' ') != NULL && strchr(target , '\"' ) != NULL)
            {
                error(400);
                continue;
            }			
            ...
            while(target[i] != '?' && i <= (line_len - 15 ));
            abs_path[i] = '\0';

            int absolutepathlen = strlen(abs_path) + 1;
            i = 0;
            char absolutepath[absolutepathlen];
            for( i = 0 ; i < absolutepathlen ;++i)
                absolutepath[i] = abs_path[i];
            absolutepath[i] = '\0';

            i = 0;

            // extract query from request-target
            char query[line_len - 15];
            if (strchr(target , '?') == NULL )
                query[0] = '\0';
            else 
            {
                ...
                {	
                    int i = 0, q = (int) (strchr(target , '?') - target );
                    do 
                    {
                        query[i] = target[q + i + 1];
                        i++;
                    }
                    while( q + i + 1 < (line_len - 15));
                    query[i] = '\0';
                }
            }	
            ...
            char path[root_len + absolutepathlen];
            for(int i = 0 ; i < root_len;++i)
                path[i] = root[i] ; 
            for(int i = 0 ; i < absolutepathlen ;++i)
                path[root_len + i] = absolutepath[i];
            path[root_len + absolutepathlen] = '\0';

            // ensure path exists
            int file_ok = access(path , F_OK);
            if (file_ok != 0)
            {
                error(404);
                continue;
            ...
            int file_read = access(path , R_OK);
            if (file_read != 0)
            {
                error(403);
                continue;
            ...
            int counter = (int) (last_dot - path);

            char extension[6];
            for(i = 0 ; i < (path_len - counter - 1) ;i++)
                extension[i] = path[counter + 1 + i];
            extension[i] = '\0';
...
const char* lookup(const char* extension)
{
    int length_ext = strlen(extension), i;
    char new_ext[20];
    for(i = 0 ; i < length_ext ;++i)
        new_ext[i] = tolower(extension[i]);

    new_ext[i] = '\0';

    if (strcmp("css" , new_ext) == 0)
        return "text/css\0"; 
    else if(strcmp("html\0" , new_ext) == 0)
        return "text/html\0";
    else if(strcmp("gif" , new_ext) == 0)
        return "image/gif\0";
    else if(strcmp("ico" , new_ext) == 0)
        return "image/x-icon\0";
    else if(strcmp("jpg" , new_ext) == 0)
        return "image/jpeg\0";
    else if(strcmp("js" , new_ext) == 0) 
        return "text/javascript\0";
    else if(strcmp("png" , new_ext) == 0)
        return "image/png\0";
    else
        return NULL;
}
